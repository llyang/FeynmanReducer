...
#if FireFly_VERSION_MAJOR != 2 || FireFly_VERSION_MINOR != 0 || \
    FireFly_VERSION_RELEASE != 3
#error "FeynmanReducer's Reconstructor overlay requires FireFly 2.0.3"
#endif
...
namespace firefly {
  ...
  private:
    ...
    // start_first_runs queues worker probes before it has populated reconst.
    // Selected evaluation is safe only after the complete output list is
    // published; otherwise an early worker could mistake a partial list for
    // the set of functions that still need values.
    std::atomic<bool> selected_black_box_ready = {false};
    ...
    struct ActiveOutputSnapshot {
      uint32_t prime_epoch = 0;
      uint64_t revision = 0;
      std::vector<uint32_t> outputs;
    };
    core::AtomicSharedPtr<const ActiveOutputSnapshot> active_output_snapshot;
    std::atomic<uint64_t> active_output_revision = {0};
    std::atomic<bool> active_output_snapshot_dirty = {false};
    struct ComputedProbe {
      std::vector<uint64_t> indices;
      std::vector<std::vector<FFInt>> dense_values;
      std::shared_ptr<const ActiveOutputSnapshot> active_output_snapshot;
      std::vector<FFInt> compact_values;
      bool compact_scalar = false;
    };
    std::queue<ComputedProbe> computed_probes;
    ...
    */
    ...
    void invalidate_active_output_snapshot();
    void publish_active_output_snapshot();
    std::shared_ptr<const ActiveOutputSnapshot> selected_black_box_outputs();
    /**
     *  Computes N probes
     *  @param lock_probe_queue the locked mutex of the queue of probes
     */
  ...
  void Reconstructor<BlackBoxTemp>::reconstruct(uint32_t prime_counter) {
    ...
    if (!done) {
      if (resume_from_state) {
        active_output_snapshot_dirty.store(false, std::memory_order_release);
        publish_active_output_snapshot();
      }
  ...
  void Reconstructor<BlackBoxTemp>::start_first_runs(bool first) {
    ...
    ComputedProbe initial_probe;
    get_probe(initial_probe);
    if (initial_probe.compact_scalar) {
      throw std::runtime_error("initial FireFly probe unexpectedly used compact output layout");
    }
    auto& indices = initial_probe.indices;
    auto& probes = initial_probe.dense_values;
  ...
  void Reconstructor<BlackBoxTemp>::run_until_done(uint32_t prime_counter) {
    ...
    while (!done) {
      ...
      ComputedProbe computed_probe;
      get_probe(computed_probe);
  ...
  template<typename BlackBoxTemp>
  void Reconstructor<BlackBoxTemp>::get_probe(ComputedProbe& probe) {
    {
      std::unique_lock<std::mutex> lock_future(future_control);

      condition_future.wait(lock_future, [this](){return computed_probes.size() != 0;});

      probe = std::move(computed_probes.front());
      computed_probes.pop();
    }

    probes_fed += probe.indices.size();

    std::lock_guard<std::mutex> lock_probe_queue(mutex_probe_queue);

    probes_queued -= static_cast<uint32_t>(probe.indices.size());
  }

  template<typename BlackBoxTemp>
  void Reconstructor<BlackBoxTemp>::feed_job(const ComputedProbe& probe) {
    const auto& indices = probe.indices;
    const auto& probes = probe.dense_values;
    if (probe.compact_scalar
        && (indices.size() != 1
            || !probe.active_output_snapshot
            || probe.active_output_snapshot->prime_epoch != prime_it
            || probe.active_output_snapshot->outputs.size()
                 != probe.compact_values.size())) {
      throw std::runtime_error("invalid compact scalar FireFly probe shape");
    }
    const std::vector<uint32_t>* active_outputs = probe.compact_scalar
      ? &probe.active_output_snapshot->outputs
      : nullptr;
    const uint32_t supplied_items = probe.compact_scalar
      ? static_cast<uint32_t>(active_outputs->size())
      : items;
    {
      std::lock_guard<std::mutex> lock(feed_control);

      interpolate_jobs += supplied_items;
    }
    ...
    for (auto & rec : reconst) {
      const uint64_t output = std::get<0>(rec);
      const FFInt* compact_value = nullptr;
      if (probe.compact_scalar) {
        while (compact_position < active_outputs->size()
               && (*active_outputs)[compact_position] < output) {
          ++compact_position;
        }
        if (compact_position == active_outputs->size()
            || (*active_outputs)[compact_position] != output) continue;
        compact_value = &probe.compact_values[compact_position];
      }
      if (std::get<1>(rec) == RECONSTRUCTING) {
        ...
        if (!done_prime.first) {
          if (done_prime.second == prime_it) {
            auto interpolate_and_write = probe.compact_scalar
              ? std::get<2>(rec)->feed(t_vec.front(), *compact_value,
                                       zi_order_vec.front(), prime_it)
              : std::get<2>(rec)->feed(t_vec, probes[output], zi_order_vec,
                                       prime_it);
    ...
    interpolate_jobs -= (supplied_items - counter);
    --feed_jobs;
      ...
      if (std::get<0>(interpolated_done_prime)) { // interpolated
        ...
        if (!std::get<1>(interpolated_done_prime)) { // not done
          if (std::get<2>(interpolated_done_prime) > prime_it) { // compare prime counters
            active_output_snapshot_dirty.store(true, std::memory_order_release);
            {
              std::lock_guard<std::mutex> lock_status(status_control);
...
/*        case 256:
  ...
  template<typename BlackBoxTemp>
  void Reconstructor<BlackBoxTemp>::invalidate_active_output_snapshot() {
    selected_black_box_ready.store(false, std::memory_order_release);
    active_output_snapshot.store(nullptr, std::memory_order_release);
    active_output_snapshot_dirty.store(false, std::memory_order_release);
  }

  template<typename BlackBoxTemp>
  void Reconstructor<BlackBoxTemp>::publish_active_output_snapshot() {
    auto snapshot = std::make_shared<ActiveOutputSnapshot>();
    snapshot->prime_epoch = prime_it;
    {
      std::lock_guard<std::mutex> lock_clean(clean);
      snapshot->outputs.reserve(reconst.size());
      for (auto& rec : reconst) {
        if (std::get<1>(rec).load() != RECONSTRUCTING) {
          continue;
        }
        const std::pair<bool, uint32_t> done_prime =
          std::get<2>(rec)->get_done_and_prime();
        if (!done_prime.first && done_prime.second == prime_it) {
          const uint64_t output = std::get<0>(rec);
          if (output > std::numeric_limits<uint32_t>::max()) {
            throw std::runtime_error("black-box output index exceeds selected-evaluation encoding");
          }
          snapshot->outputs.emplace_back(static_cast<uint32_t>(output));
        }
      }
    }
    std::sort(snapshot->outputs.begin(), snapshot->outputs.end());
    snapshot->revision = active_output_revision.fetch_add(1, std::memory_order_relaxed) + 1;
    active_output_snapshot.store(std::move(snapshot), std::memory_order_release);
    selected_black_box_ready.store(true, std::memory_order_release);
  }

  template<typename BlackBoxTemp>
  std::shared_ptr<const typename Reconstructor<BlackBoxTemp>::ActiveOutputSnapshot>
  Reconstructor<BlackBoxTemp>::selected_black_box_outputs() {
    if (!selected_black_box_ready.load(std::memory_order_acquire)) {
      return {};
    }
    if (active_output_snapshot_dirty.exchange(false, std::memory_order_acq_rel)) {
      publish_active_output_snapshot();
    }
    auto snapshot = active_output_snapshot.load(std::memory_order_acquire);
    if (!snapshot || snapshot->prime_epoch != prime_it) {
      throw std::logic_error("active-output snapshot belongs to a different prime epoch");
    }
    return snapshot;
  }
  ...
  void Reconstructor<BlackBoxTemp>::compute_probe(std::unique_lock<std::mutex>& lock_probe_queue) {
    ...
    std::vector<FFInt> values_vec = std::move(requested_probes.front().second);
    ...
    std::shared_ptr<const ActiveOutputSnapshot> output_snapshot;
    const bool use_selected_outputs =
      !factor_scan.load(std::memory_order_relaxed)
      && !scan.load(std::memory_order_relaxed)
      && (output_snapshot = selected_black_box_outputs());
    const std::vector<uint32_t>* active_outputs =
      output_snapshot ? &output_snapshot->outputs : nullptr;
    std::vector<FFInt> probe;
    bool compact_scalar = false;
    if (use_selected_outputs) {
      if constexpr (requires(BlackBoxTemp& black_box,
                             const std::vector<FFInt>& values,
                             const std::vector<uint32_t>& outputs) {
                      black_box.eval_selected_compact(values, outputs);
                    }) {
        probe = static_cast<BlackBoxTemp&>(bb).eval_selected_compact(values_vec, *active_outputs);
        compact_scalar = true;
      } else if constexpr (requires(BlackBoxTemp& black_box,
                                    const std::vector<FFInt>& values,
                                    const std::vector<uint32_t>& outputs) {
                             black_box.eval_selected(values, outputs);
                           }) {
        probe = static_cast<BlackBoxTemp&>(bb).eval_selected(values_vec, *active_outputs);
      } else {
        probe = bb.eval(values_vec);
      }
    } else {
      probe = bb.eval(values_vec);
    }
    ...
    if (compact_scalar) {
      if (probe.size() != active_outputs->size()) {
        throw std::runtime_error("compact selected black-box result has wrong size");
      }
      for (size_t position = 0; position != probe.size(); ++position) {
        const uint32_t output = (*active_outputs)[position];
        const auto factor = parsed_factors.find(output);
        if (!factor_scan && factor != parsed_factors.end()) {
          auto res = factor->second.evaluate_pre(values_vec);
          probe[position] /= res[0];
        }
      }
    } else {
      for (size_t i = 0; i != probe.size(); ++i) {
        if (!factor_scan && parsed_factors.find(i) != parsed_factors.end()
            && (!use_selected_outputs
                || std::binary_search(active_outputs->begin(), active_outputs->end(),
    ...
    computed.indices = std::move(indices);
    computed.compact_scalar = compact_scalar;
    if (compact_scalar) {
      computed.active_output_snapshot = std::move(output_snapshot);
      computed.compact_values = std::move(probe);
    } else {
      computed.dense_values.reserve(probe.size());
      for (auto& value : probe) {
        computed.dense_values.emplace_back(std::vector<FFInt>(1, value));
      }
    }
    ...
    computed_probes.emplace(std::move(computed));
  ...
  void Reconstructor<BlackBoxTemp>::compute_probe(std::unique_lock<std::mutex>& lock_probe_queue) {
    ...
    ComputedProbe computed;
    computed.indices = std::move(indices);
    computed.dense_values = std::move(tmp);
    computed_probes.emplace(std::move(computed));
  ...
  void Reconstructor<BlackBoxTemp>::reset_new_prime() {
    ...
    requested_probes = std::deque<std::pair<uint64_t, std::vector<FFInt>>>();
    computed_probes = std::queue<ComputedProbe>();
  }
  ...
  void Reconstructor<BlackBoxTemp>::load_precomputed_probes_from_file() {
    ...
    if (ifile.is_open()) {
      ...
      while (std::getline(gzfile, line)) {
        ...
        ComputedProbe computed;
        computed.indices.emplace_back(ind);
        computed.dense_values = std::move(probes);
        computed_probes.emplace(std::move(computed));
        index_map.emplace(std::make_pair(ind, std::make_pair(t, zi_order)));
        ++ind;
        ++probes_queued;
    ...
    while (true) {
      ...
      if (done && !scan) {
...

        {
          ...
          ComputedProbe computed;
          computed.indices = std::move(indices);
          computed.dense_values = std::move(results);
          computed_probes.emplace(std::move(computed));
